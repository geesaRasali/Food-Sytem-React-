  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '14px' }}>
        ...
        >
          <img src={assets.parcel_icon} alt="Order" style={{ width: 40, height: 40, objectFit: 'contain' }} />
        </div>

        <div style={{ flex: 1, minWidth: 0 }}>
          {/* Items list */}
          <p style={{ fontWeight: 600, fontSize: '14px', color: '#1f2937', marginBottom: 4 }}>
          ...
          >
            <span
              style={{
                ...
                animation: order.status !== 'Delivered' ? 'myOrdersPulse 1.8s infinite' : 'none',
              }}
            />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 1, flex: '0 0 auto' }}>
                  <div
                    style={{
                      ...
                      border: isCurrent ? `3px solid ${step.color}` : '2px solid ' + (isCompleted ? step.color : '#d1d5db'),
  ...
  const fetchOrders = useCallback(async (silent = false) => {
    try {
      ...
      if (response.data.success) {
        ...
        if (!silent) setError(response.data.message || 'Failed to fetch orders');
      }
    } catch (err) {
    ...
    return (
      <>
        ...
        <div style={{ margin: '48px 0' }}>
          <h2 style={{ marginBottom: 30, fontSize: 24, fontWeight: 800, color: '#111827' }}>My Orders</h2>
  ...
  if (error) {
    return (
      <div style={{ margin: '48px 0' }}>
        <h2 style={{ marginBottom: 30, fontSize: 24, fontWeight: 800, color: '#111827' }}>My Orders</h2>
        <div style={{
          border: '1px solid #fca5a5', background: '#fff5f5', borderRadius: 12,
          padding: '40px 20px', textAlign: 'center', color: '#ef4444', fontSize: 15,
        }}>
          {error}
        </div>
      </div>
    ...
    <>
      ...
      <div style={{ margin: '48px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 10 }}>
          <div>
            <h2 style={{ fontSize: 26, fontWeight: 800, color: '#111827', margin: 0 }}>My Orders</h2>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            ...
            >
              <svg
                ...
                style={{ animation: isRefreshing ? 'myOrdersSpin 0.8s linear infinite' : 'none' }}
              >
        ...
        {data.length === 0 ? (
          <div style={{
            ...
            <img src={assets.parcel_icon} alt="No orders" style={{ width: 56, opacity: 0.3, marginBottom: 14 }} />
            <p style={{ margin: 0, fontWeight: 600 }}>No orders yet</p>
            <p style={{ margin: '6px 0 0', fontSize: 13 }}>Start ordering some delicious food!</p>
          </div>
        ) : (
          <div style={{ animation: 'myOrdersFadeIn 0.35s ease' }}>
            {data.map((order, index) => (
              <OrderCard key={order._id || index} order={order} onTrack={handleTrack} />
            ))}
          </div>
        )}
      </div>
